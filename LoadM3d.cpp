						...
						std::vector<M3dMaterial>& mats)
{
	...
	if( fin )
	{
		fin >> ignore; // tekst nag³ówka pliku
		...
		ReadSubsetTable(fin, numSub, subsets);
...
void M3DLoader::ReadMaterials(std::ifstream& fin, UINT numMaterials, std::vector<M3dMaterial>& mats)
{
     ...
     fin >> ignore; // tekst nag³ówka - materia³y
...
void M3DLoader::ReadSubsetTable(std::ifstream& fin, UINT numSubsets, std::vector<MeshGeometry::Subset>& subsets)
{
	...
	fin >> ignore; // tekst nag³ówka - podzbiory
	for(UINT i = 0; i < numSubsets; ++i)
	{
		...
		fin >> ignore >> subsets[i].MaterialNum;
    }
}
...
void M3DLoader::ReadVertices(std::ifstream& fin, UINT numVertices, std::vector<Vertex>& vertices)
{
    ...
    fin >> ignore; // tekst nag³ówka - wierzcho³ki
...
void M3DLoader::ReadTriangles(std::ifstream& fin, UINT numTriangles, std::vector<int>& indices)
{
	std::string ignore;
    indices.resize(numTriangles*3);

    fin >> ignore; // tekst nag³ówka - trójk¹ty
